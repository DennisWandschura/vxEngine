...
void drawQuads(in VSOutput input, in float3 vmin, in float3 vmax, inout TriangleStream< GSOutput > output)
{
	float l = vmin.x;
	float r = vmax.x;
	float b = vmin.y;
	...
	GSOutput element;
	element.lightIndex = input.lightIndex;
	...
	// bottom
	positions[0] = mul(camera.pvMatrix, float4(l, t, n, 1));
	positions[1] = mul(camera.pvMatrix, float4(r, t, n, 1));
	positions[2] = mul(camera.pvMatrix, float4(r, t, f, 1));
	positions[3] = mul(camera.pvMatrix, float4(l, t, f, 1));
	emitQuad(element, positions, output);
	...
	positions[0] = mul(camera.pvMatrix, float4(l, b, f, 1));
	positions[1] = mul(camera.pvMatrix, float4(r, b, f, 1));
	positions[2] = mul(camera.pvMatrix, float4(r, b, n, 1));
	positions[3] = mul(camera.pvMatrix, float4(l, b, n, 1));
	emitQuad(element, positions, output);
}
	...
	)
{
	...
	drawQuads(input[0], vmin, vmax, output);
